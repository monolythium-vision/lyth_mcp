...
import {
  ...
  assetRisk,
  evaluateAssetUseCase,
  getAsset,
  listAssets,
  loadAssetRegistry,
  privateDenominationWarning,
...
import {
  ...
  merchantPolicyStoreInfo,
  removeMerchantPolicy,
  upsertMerchantPolicy,
} from "./merchant_policy.js";
import {
  diffRunbookContent,
  getCanonicalRunbook,
...
import {
  ...
  changenowSwapStatus,
  changenowSwapList,
  changenowFiatEstimate,
  changenowFiatSellDraft,
  changenowRedactedConfig,
} from "./changenow.js";
...
interface RunbookDraft {
  ...
  runbook: RunbookName;
  title: string;
  status: "draft" | "live_preparable" | "draft_only";
  createdAt: string;
  ...
  liveExecution: {
    supported: boolean;
    mode: "wallet_approval" | "signed_broadcast" | "not_live";
    reason: string;
  };
...
function isWireAddress(value: string): boolean {
  return /^0x[0-9a-fA-F]{40}$/.test(value);
}

async function resolveRecipient(value: string): Promise<ResolvedRecipient | null> {
...
function parseFlexibleBigint(value: string): bigint {
...
function compareDecimal(a: string, b: string, decimals = 18): number {
  const aa = decimalToUnits(a, decimals);
  const bb = decimalToUnits(b, decimals);
  return aa === bb ? 0 : aa > bb ? 1 : -1;
}

function apiBaseFromRpc(endpoint: string): string {
...
async function apiGet<T>(endpoint: string, path: string, query: Record<string, string | number | undefined> = {}): Promise<T> {
...
async function sendConnectorJson(connector: ConnectorRecord, payload: Record<string, unknown>) {
...
function connectorResponseReference(responseBody: unknown, fallback: string): string {
...
async function probeEndpoint(endpoint: string) {
  ...
  try {
    const chainId = parseQuantity(await rpcCall<string>(endpoint, "eth_chainId"));
    const height = parseQuantity(await rpcCall<string>(endpoint, "eth_blockNumber"));
    return {
      endpoint,
...
async function firstReachableEndpoint(): Promise<string> {
  ...
  const selected = probes
    .filter((p): p is Awaited<ReturnType<typeof probeEndpoint>> & { ok: true; height: string } => p.ok === true)
...
function buildRunbookDraft(args: {
  ...
  const walletRequests: Array<Record<string, unknown>> = [];
  const risks = [
...
function validateRunbook(draft: RunbookDraft) {
...
function prepareWalletRequest(draft: RunbookDraft, from?: string) {
...
}): Promise<{ registry: Awaited<ReturnType<typeof loadVendorRegistry>>; vendor: VendorRecord; quote: VendorQuote }> {
...
async function evaluateVendorRisk(vendor: VendorRecord, quote?: VendorQuote) {
  const policy = await getMerchantPolicy(vendor.id);
  return evaluateMerchantPolicy({ vendor, quote, policy });
}

async function evaluateAssetPolicy(symbol: string, useCase: AssetUseCase) {
...
function short(value: string | undefined, left = 6, right = 4): string {
  if (!value) {
    return "";
  }
...
server.tool(
  "agent_wallet_create",
  "Create an explicit low-value agent operating wallet with user-approved purpose and caps.",
  {
...
server.tool(
  "agent_wallet_fund_request",
  "Draft a human-readable request to fund an agent wallet for a bounded task.",
  {
    name: z.string().min(1),
  ...
  async ({ name, amount, asset, purpose, expiresAt }) => {
    ...
    const draft = buildRunbookDraft({
      ...
      agent: {
        name,
        address: wallet.address,
        purpose: wallet.agent && typeof wallet.agent === "object" ? (wallet.agent as { purpose?: string }).purpose : undefined,
      },
    });
    const receipt = await addReceipt({
...
server.tool(
  "agent_wallet_limits",
  "Update explicit limits and metadata for an agent operating wallet.",
  {
    name: z.string().min(1),
  ...
  async ({ name, lowValueMaxAmount, lowValueDailyLimit, maxBalance, allowedCounterparties, allowedCategories, expiresAt, fallbackApproval }) => {
    ...
    if (lowValueMaxAmount || lowValueDailyLimit) {
      wallet = await configureLowValuePolicy({
        name,
        enabled: true,
        maxAmount: lowValueMaxAmount ?? String(wallet.lowValue && typeof wallet.lowValue === "object" ? (wallet.lowValue as { maxAmount?: string }).maxAmount ?? DEFAULT_LOW_VALUE_MAX : DEFAULT_LOW_VALUE_MAX),
        dailyLimit: lowValueDailyLimit ?? (wallet.lowValue && typeof wallet.lowValue === "object" ? (wallet.lowValue as { dailyLimit?: string }).dailyLimit : undefined),
      });
    }
...
server.tool(
  "agent_wallet_pause",
  "Pause an agent wallet by disabling low-value local signing and marking the wallet paused.",
  {
    name: z.string().min(1),
...
server.tool(
  "agent_wallet_drain",
  "Prepare and optionally sign a transfer that drains an agent wallet back to a principal or recovery address.",
  {
    name: z.string().min(1),
    to: z.string().describe("0x recipient address or exact addressbook contact name."),
...
server.tool(
  "agent_wallet_delete",
  "Delete a local agent wallet record after explicit confirmation.",
  {
    name: z.string().min(1),
...
server.tool(
  ...
  async ({ apiKey, privateApiKey, partnerCode, defaultRefundAddress }) => {
    ...
    return text({
      ...
      warning:
        "ChangeNow swaps move real funds. Inspect each swap_create payload before broadcasting deposits.",
    });
  },
);
...
server.tool(
  "changenow_status",
  "Probe ChangeNow API reachability with the configured key.",
  {},
  async () => text(await changenowStatus()),
);

server.tool(
  "changenow_currencies",
  "List ChangeNow currencies (filterable by active / flow / buy / sell).",
  {
...
server.tool(
  "changenow_min_amount",
  "Get the minimum swappable amount for a pair.",
  {
...
server.tool(
  "changenow_estimate",
  "Quote a swap. Pass `fromAmount` for direct, or `toAmount` for reverse. `flow: 'fixed-rate'` returns a rateId valid for ~20 minutes that you pass back into changenow_swap_create.",
  {
    fromCurrency: z.string().min(2),
    toCurrency: z.string().min(2),
...
server.tool(
  "changenow_swap_create",
  "Create a swap. Returns the deposit address (payinAddress) — send fromCurrency to that address and ChangeNow forwards toCurrency to payoutAddress. Fixed-rate swaps require the rateId returned by changenow_estimate.",
  {
    fromCurrency: z.string().min(2),
    toCurrency: z.string().min(2),
...
server.tool(
  "changenow_swap_status",
  "Poll the status of a swap by ChangeNow swap id.",
  { id: z.string().min(4) },
  async ({ id }) => text(await changenowSwapStatus(id)),
);

server.tool(
  "changenow_swap_list",
  "List historic swaps (paginated).",
  {
...
server.tool(
  "changenow_fiat_estimate",
  "Quote a crypto → fiat (or fiat → crypto) sale. Off-ramp; KYC will be required at sell time.",
  {
    fromCurrency: z.string().min(2),
    toCurrency: z.string().min(2),
    fromAmount: z.number().positive().optional(),
...
server.tool(
  "changenow_fiat_sell_draft",
  "Draft a crypto-to-fiat sell payload. DOES NOT submit — fiat off-ramp requires KYC and irreversible bank transfers. Review the returned `draft.body` and submit manually after KYC clears.",
  {
    fromCurrency: z.string().min(2),
    toCurrency: z.string().min(2),
...
server.tool(
  "changenow_config_redacted",
  "Inspect the ChangeNow connector config (no secrets revealed).",
  {},
  async () => text({ config: await changenowRedactedConfig() }),
);
...
server.tool(
  "profile_create",
  "Create an encrypted traveler profile (legal name, DOB, passport, contact, ticket delivery email, frequent-flyer numbers). PII is encrypted at rest with AES-256-GCM + scrypt; only a redacted preview is visible to list/get.",
  {
  ...
  async ({ id, displayName, profile, passphrase, allowLocalKey, overwrite }) => {
    const summary = await createProfile({
      id,
      displayName,
      profile: profile as ProfilePlaintext,
      passphrase,
      allowLocalKey,
      overwrite,
    });
...
server.tool(
  "profile_update",
  "Patch an existing traveler profile. Only the fields you supply are changed; the rest are preserved.",
  {
    id: z.string().min(1),
  ...
  async ({ id, displayName, patch, passphrase }) => {
    const summary = await updateProfile({
      id,
      displayName,
      patch: patch as Partial<ProfilePlaintext>,
      passphrase,
    });
...
server.tool(
  "profile_list",
  "List saved traveler profiles. Returns redacted previews only (no plaintext PII).",
  {},
  async () => text({ profiles: await listProfiles() }),
);

server.tool(
  "profile_get",
  "Get one traveler profile by id. Returns redacted preview only — call profile_reveal for plaintext.",
  { id: z.string().min(1) },
  async ({ id }) => text({ profile: await getProfile(id) }),
);

server.tool(
  "profile_reveal",
  "Reveal the full plaintext of an encrypted traveler profile. Requires explicit confirmation; do not paste output into shared contexts.",
  {
    id: z.string().min(1),
  ...
  async ({ id, passphrase }) => {
    ...
    return text({
      ...
      profile,
      warning:
        "This response contains plaintext PII (passport, DOB, contact). Treat as a secret. Do not paste into chat history or commit it.",
    });
  },
);
...
server.tool(
  "profile_delete",
  "Delete a traveler profile after explicit confirmation.",
  {
    id: z.string().min(1),
...
server.tool(
  "profile_store_info",
  "Show profile store path, count, and file mode for diagnostics.",
  {},
...
server.tool(
  "travala_book_pay",
  "Book a Travala package and pay the x402 invoice with the configured Base USDC wallet. Customer fields can be supplied directly or pulled from a saved secure profile (profileId). Reuses the x402 vendor policy 'travala' and the locally configured ERC-8004 agentId / rewardWallet for cbBTC attribution.",
  {
...
server.tool(
  "addressbook_add",
  "Add or update a local MCP addressbook contact. Use this before sending to named contacts.",
  {
    name: z.string().min(1),
...
server.tool(
  "addressbook_lookup",
  "List or search local MCP addressbook contacts.",
  {
...
server.tool(
  "addressbook_remove",
  "Remove a local MCP addressbook contact by name.",
  {
    name: z.string().min(1),
  },
...
server.tool(
  "wallet_configure_low_value",
  "Enable or disable no-passphrase low-value signing for a local agent wallet.",
  {
    name: z.string().min(1),
    enabled: z.boolean(),
...
server.tool(
  ...
  "Reveal a wallet mnemonic after passphrase decryption. Use only for backup; never paste it into chats or logs.",
  {
    name: z.string().min(1),
    passphrase: z.string().min(12).optional(),
...
server.tool(
  "wallet_delete",
  "Delete a local MCP wallet from the wallet store.",
  {
    name: z.string().min(1),
    confirmName: z.string().min(1).describe("Must exactly equal name."),
  },
...
server.tool(
  "account_overview",
  "Get live account balance, nonce, profile, and recent flow for a Monolythium address.",
  {
...
server.tool(
  "recent_transactions",
  "Get recent live transactions from lyth_txFeed.",
  {
...
server.tool(
  "tx_lookup",
  "Look up a transaction by hash, including status, receipt, decoded view, and raw transaction where available.",
...
server.tool(
  ...
  async ({ question, format, limit }) => {
    ...
    if (/(node|tpm|pcr|attestation|hosting class|hosting risk)/i.test(question)) {
      ...
      const role: NodeRole | undefined = /prover/i.test(question)
        ? "prover"
    ...
    if (/(cluster|operator|validator|prover|gpu|proof|zkml|foundation|decentralization|decentralisation|stake|staking|delegation cap|stake cap|over[-\s]?cap|taper|monarch|quorum|service roi|resource pressure|archive service|oracle service)/i.test(question)) {
      ...
      if (/autovote|rebalance|undelegate|delegate|stake status/i.test(question)) {
        const mode: DelegationMode = /yield/i.test(question)
          ? "max_yield"
...
server.tool(
  "search_chain",
  "Search live chain data for addresses, hashes, blocks, clusters, or labels.",
  {
    query: z.string(),
...
server.tool(
  "markets",
  "Read live CLOB markets and optional market details/order book/trades.",
  {
    limit: z.number().min(1).max(100).optional(),
...
server.tool(
  "draft_runbook",
  "Draft a typed AI runbook for payment, booking, escrow, trading, policy, receipt, or vendor-rating workflows.",
  {
    runbook: runbookEnum,
...
server.tool(
  "validate_runbook",
  "Validate a drafted runbook against spending-policy and MCP safety rules.",
  {
    runbook: runbookEnum,
    fields: recordSchema,
    policy: policySchema,
...
server.tool(
  "prepare_wallet_request",
  "Prepare a wallet approval payload from a runbook. MVP supports native LYTH pay_vendor transfers.",
  {
    runbook: runbookEnum,
...
server.tool(
  "tx_outbox_list",
  "List local signed payloads that can be retried without rebuilding/re-signing.",
  {
  ...
  async ({ status, walletName, limit }) => text({
    ...
    entries: await listOutboxEntries({ status: status as OutboxStatus | undefined, walletName, limit }),
  }),
);

server.tool(
  "tx_outbox_get",
  "Get a local outbox entry by id.",
  { id: z.string().min(1) },
  async ({ id }) => text(await getOutboxEntry(id)),
);
...
server.tool(
  "tx_outbox_forget",
  "Remove a local outbox entry. This does not invalidate an already-signed payload.",
  {
    id: z.string().min(1),
...
server.tool(
  "tx_status_summary",
  "Summarize live transaction status by tx hash or local outbox id.",
  {
    txHash: z.string().optional(),
    outboxId: z.string().optional(),
  },
...
server.tool(
  "receipt_list",
  "List local MCP receipts for drafted, signed, submitted, confirmed, or failed operations.",
  {
  ...
  async ({ status, kind, walletName, limit }) => text({
    ...
    receipts: await listReceipts({ status: status as ReceiptStatus | undefined, kind, walletName, limit }),
  }),
);

server.tool(
  "receipt_get",
  "Get one local MCP receipt by id.",
  { id: z.string().min(1) },
  async ({ id }) => text(await getReceipt(id)),
);

server.tool(
  "receipt_export",
  "Export a local MCP receipt as JSON.",
  { id: z.string().min(1) },
  async ({ id }) => text({
...
server.tool(
  "mcp_dashboard",
  "Render a compact Markdown dashboard for Claude Code or other text UIs.",
  {
  ...
  async ({ includeEntries }) => {
    ...
    if (includeEntries !== false) {
      lines.push(
        ...
        connectors.length
          ? mdTable(
              ["ID", "Vendor", "Enabled", "Auth", "Endpoint"],
        ...
        bookings.length
          ? mdTable(
              ["ID", "Status", "Vendor", "Service", "Amount"],
        ...
        "",
        "## Merchant Policies",
        merchantPolicies.length
          ? mdTable(
              ["Vendor", "Enabled", "Allow", "Deny", "Cap", "Assets"],
...
server.tool(
  "vendor_search",
  "Search the local vendor registry used by agent runbooks. Set LYTH_MCP_VENDOR_REGISTRY to a JSON file.",
  {
...
server.tool(
  "asset_search",
  "Search local asset metadata and wallet-readable risk labels.",
  {
    query: z.string().optional(),
  ...
  async ({ query, kind, denomination, status, useCase, limit }) => {
    ...
    const assets = listAssets(registry.registry, {
      ...
      denomination: denomination as AssetDenomination | undefined,
      status: status as AssetStatus | undefined,
      useCase: useCase as AssetUseCase | undefined,
      limit,
    });
    return text({
      registry: assetRegistrySummary(registry),
...
server.tool(
  "asset_get",
  "Get one asset's metadata, risk labels, allowed use cases, and privacy/bridge warnings.",
  {
...
server.tool(
  "asset_risk_label",
  "Render wallet-readable risk labels for an asset and optional intended use case.",
  {
    symbol: z.string().min(1),
    useCase: assetUseCaseEnum.optional(),
  },
  async ({ symbol, useCase }) => {
    ...
    return text({
      asset,
      risk: assetRisk(asset),
      policy: useCase ? evaluateAssetUseCase(asset, useCase as AssetUseCase) : undefined,
    });
  },
);
...
server.tool(
  "asset_route_labels",
  "Show asset route labels by joining local asset metadata with bridge route metadata.",
  {
    symbol: z.string().min(1),
  },
  async ({ symbol }) => {
...
server.tool(
  "privacy_policy_check",
  "Check whether an asset/denomination can be used for a requested action. Private LYTH is blocked from productive/public actions.",
  {
    symbol: z.string().min(1),
  ...
  async ({ symbol, useCase }) => {
    const registry = await loadAssets();
    const asset = getAsset(registry.registry, symbol);
    const policy = evaluateAssetUseCase(asset, useCase as AssetUseCase);
    return policy.ok ? text(policy) : errorJson(policy);
  },
);

server.tool(
  "private_denomination_warning",
  "Explain public/private LYTH separation, one-way privacy crossing, and blocked productive use cases.",
  {
...
server.tool(
  "commerce_safety_check",
  "Check a vendor/service/search request against local anti-illicit-commerce policy before discovery, orders, or bookings.",
  {
    query: z.string().optional(),
    category: z.string().optional(),
...
server.tool(
  "risk_explain",
  "Render a plain-English risk summary for a planned MCP action using policy, bridge, asset, commerce, and preflight inputs.",
  {
...
server.tool(
  "contract_path_guidance",
  "Explain Monolythium's no-EVM contract path for Solidity/EVM/Rust/RISC-V requests.",
  {
...
server.tool(
  "bridge_routes",
  "List configured bridge/liquidity routes with status, cooldown, and trust model metadata.",
  {
    asset: z.string().optional(),
  ...
  async ({ asset, sourceChain, destinationChain, status, routeType, limit }) => {
    ...
    return text({
      ...
      routes: listBridgeRoutes(registry.registry, {
        ...
        sourceChain,
        destinationChain,
        status: status as BridgeRouteStatus | undefined,
        routeType: routeType as BridgeRouteType | undefined,
        limit,
      }),
...
server.tool(
  "bridge_route_get",
  "Get one bridge route with trust assumptions, cooldown, drain cap, and circuit-breaker metadata.",
  {
...
server.tool(
  "bridge_quote",
  "Preflight a bridge amount against route status, cooldown, fees, drain caps, and circuit breakers. Does not build or submit a bridge transaction.",
  {
    amount: z.string(),
    asset: z.string(),
...
server.tool(
  "bridge_cooldown_matrix",
  "Show the configured cooldown matrix for IBC, zk-light-client, Bitcoin, Solana, Ethereum, and trusted routes.",
  {
    asset: z.string().optional(),
  },
  async ({ asset }) => {
...
server.tool(
  "liquidity_onboarding",
  "Explain how to bring an asset into Mono using configured bridge/liquidity routes.",
  {
    asset: z.string(),
    sourceChain: z.string().optional(),
    amount: z.string().optional(),
...
server.tool(
  "delegation_cap_explain",
  "Explain current delegation phase, per-cluster cap, minimum diversification, over-cap grace, and tapered rewards.",
  {
  ...
  async (args) => text({
    phaseConfig: delegationPhaseConfig((args.phase as DelegationPhase | undefined) ?? "growth"),
    explanation: explainDelegationCaps({
      ...args,
      phase: args.phase as DelegationPhase | undefined,
    }),
  }),
);
...
server.tool(
  "stake_status",
  "Summarize local staking positions against delegation phase caps and cluster risk metadata. Planning only.",
  {
    phase: delegationPhaseEnum.optional(),
  ...
  async ({ phase, positions }) => {
    ...
    return text({
      ...
      status: stakeStatus(registry.registry, {
        phase: phase as DelegationPhase | undefined,
        positions: positions as DelegationPosition[] | undefined,
      }),
    });
  },
);
...
server.tool(
  "delegate_draft",
  "Draft a local delegation plan for one cluster. Does not build or submit a staking transaction.",
  {
    clusterId: z.string().min(1),
    amount: z.string().min(1),
  ...
  async ({ clusterId, amount, mode, phase, positions }) => {
    ...
    return text({
      ...
      draft: delegateDraft(registry.registry, {
        clusterId,
        amount,
        mode: mode as DelegationMode | undefined,
        phase: phase as DelegationPhase | undefined,
        positions: positions as DelegationPosition[] | undefined,
      }),
    });
...
server.tool(
  "rebalance_draft",
  "Draft a local rebalance plan across clusters for yield, diversity, or decentralization mode. Planning only.",
  {
    mode: delegationModeEnum.optional(),
    phase: delegationPhaseEnum.optional(),
    positions: delegationPositionsSchema,
...
server.tool(
  "undelegate_draft",
  "Draft a local undelegation plan for one cluster. Does not build or submit a staking transaction.",
  {
    clusterId: z.string().min(1),
    amount: z.string().min(1),
    positions: delegationPositionsSchema,
...
server.tool(
  "autovote_simulate",
  "Simulate cluster ranking for staking autovote modes without voting, delegating, or signing.",
  {
    mode: delegationModeEnum.optional(),
    phase: delegationPhaseEnum.optional(),
    positions: delegationPositionsSchema,
...
server.tool(
  "node_search",
  "Search local node metadata by cluster, operator, role, status, region, hosting class, attestation status, GPU, and TPM.",
  {
    query: z.string().optional(),
    clusterId: z.string().optional(),
    operatorId: z.string().optional(),
  ...
  async ({ query, clusterId, operatorId, role, status, region, hostingClass, attestationStatus, gpuRequired, tpmRequired, limit }) => {
    ...
    const nodes = listNodes(registry.registry, {
      ...
      region,
      hostingClass: hostingClass as NodeHostingClass | undefined,
      attestationStatus: attestationStatus as AttestationStatus | undefined,
      gpuRequired,
      tpmRequired,
      limit,
    });
...
server.tool(
  "node_attestation_get",
  "Get local TPM/attestation metadata for one node and compare PCRs against the expected profile.",
  {
...
server.tool(
  "node_pcr_explain",
  "Explain TPM PCR values for a node, including expected/actual PCR profile and local measured-boot meaning.",
  {
    nodeId: z.string().min(1),
...
server.tool(
  ...
  "Score local node diversity by ASN, provider, country, hosting class, operator, and cluster.",
  {
    clusterId: z.string().optional(),
    operatorId: z.string().optional(),
    region: z.string().optional(),
  ...
  async ({ clusterId, operatorId, region, role }) => {
    ...
    return text({
      registry: nodeRegistrySummary(registry),
      diversity: nodeDiversityScore(registry.registry, { clusterId, operatorId, region, role: role as NodeRole | undefined }),
    });
  },
);
...
server.tool(
  "node_hosting_class",
  "Explain one node's hosting class and correlated-failure risk.",
  {
    nodeId: z.string().min(1),
  },
  async ({ nodeId }) => {
...
server.tool(
  "provider_onboarding_draft",
  "Draft local vendor registry, merchant policy, availability, and connector metadata for a provider. Does not publish anything on-chain.",
  {
    vendorId: z.string().min(1),
...
server.tool(
  "connector_set",
  "Create or update a local encrypted webhook connector for a vendor.",
  {
...
server.tool(
  "connector_get",
  "Get a local connector without revealing its secret.",
  {
    id: z.string().min(1),
  },
...
server.tool(
  "connector_list",
  "List local webhook connectors without revealing secrets.",
  {
    vendorId: z.string().optional(),
...
server.tool(
  "connector_remove",
  "Remove a local webhook connector and its encrypted secret.",
  {
    id: z.string().min(1),
...
server.tool(
  "connector_test_webhook",
  "Preview or send a test JSON payload through a local webhook connector.",
  {
...
server.tool(
  "merchant_policy_set",
  "Create or update local merchant risk controls for a vendor.",
  {
    vendorId: z.string().min(1),
...
server.tool(
  "merchant_policy_get",
  "Get local merchant policy and risk status for a vendor.",
  {
    vendorId: z.string().min(1),
  },
  async ({ vendorId }) => {
...
server.tool(
  "merchant_policy_list",
  "List local merchant policies.",
  {
    vendorId: z.string().optional(),
...
server.tool(
  "merchant_policy_remove",
  "Remove a local merchant policy for a vendor.",
  {
    vendorId: z.string().min(1),
...
server.tool(
  "merchant_risk_check",
  "Evaluate vendor, amount, and asset against local merchant risk policy without creating an order.",
  {
    vendorId: z.string().min(1),
...
server.tool(
  "order_fulfill_manual",
  "Mark a local order fulfilled after manual vendor confirmation. This records evidence but does not call an external API.",
  {
    orderId: z.string().min(1),
...
server.tool(
  "order_fulfill_webhook",
  "Send a local order to a configured vendor webhook connector and record the fulfillment request.",
  {
    orderId: z.string().min(1),
  ...
  async ({ orderId, connectorId, allowUnpaid, allowRetry, extra }) => {
    ...
    let response: Awaited<ReturnType<typeof sendConnectorJson>>;
    try {
...
server.tool(
  "booking_request_create",
  "Create a local service-booking request and canonical book_service runbook draft.",
  {
    vendorId: z.string().min(1),
...
server.tool(
  "booking_accept_demo",
  "Mark a local booking accepted by a demo provider. No external provider is contacted.",
  {
...
server.tool(
  "booking_send_webhook",
  "Send a local service booking request to a configured vendor webhook connector.",
  {
    bookingId: z.string().min(1),
...
server.tool(
  "booking_prepare_escrow",
  "Prepare an open_escrow runbook draft for a local booking.",
  {
    bookingId: z.string().min(1),
...
server.tool(
  "booking_mark_paid",
  "Mark a booking paid after observing or supplying a payment/escrow transaction hash.",
  {
    bookingId: z.string().min(1),
    txHash: z.string().min(1),
  },
...
server.tool(
  "booking_complete_dry_run",
  "Mark a local booking completed by a dry-run fulfillment adapter.",
  {
    bookingId: z.string().min(1),
...
server.tool(
  ...
  "Open a local demo dispute for a booking.",
  {
    bookingId: z.string().min(1),
    reason: z.string().min(1),
...
server.tool(
  ...
  async ({ status, vendorId, limit }) => text({
    ...
    bookings: await listBookings({ status: status as BookingStatus | undefined, vendorId, limit }),
  }),
);

server.tool(
  "booking_cancel",
  "Cancel a local booking if it has not completed.",
  {
    bookingId: z.string().min(1),
    reason: z.string().optional(),
...
server.tool(
  "api_get",
  "Low-level read-only helper for the node /api/v1 surface. Use when a higher-level tool is missing.",
  {
