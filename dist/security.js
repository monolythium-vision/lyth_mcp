import { bridgeCircuitBreakerAlerts, bridgeStatusSummary, } from "./bridges.js";
export function securityStatus(ctx) {
    const components = [
...
export function emergencyStateWatch(ctx) {
    ...
    const failedBroadcasts = ctx.receipts.filter((receipt) => (receipt.status === "failed" &&
...
export function bridgeBlastRadiusMonitor(ctx, args = {}) {
...
export function recoveryStatus(ctx, walletName) {
...
function mempoolPosture(rpcHealth) {
...
function walletPosture(wallets, outboxEntries) {
    const hotWallets = wallets.filter((wallet) => Boolean(wallet.lowValue?.enabled));
...
function outboxPosture(entries, now) {
...
function recoveryReadiness(wallet, signed, submitted) {
    if (wallet.agent?.paused) {
        return "watch";
    }
    if (signed > 0 || submitted > 0) {
        return "degraded";
    }
    if (wallet.lowValue?.enabled) {
        return "watch";
    }
    return "ok";
}
function recoveryRunbooks(walletName) {
    return [
...
function highestSeverity(values) {
    return values.reduce((highest, value) => severityRank(value) > severityRank(highest) ? value : highest, "ok");
}
