export function accountSafetyProfiles(args) {
    ...
    const highest = profiles.reduce((level, profile) => higherRisk(level, profile.risk.level), "low");
    return {
...
export function simulateHotWalletPolicy(args) {
...
export function explainWalletThresholds(args = {}) {
    const asset = (args.asset ?? "LYTH").toUpperCase();
...
function accountSafetyProfile(wallet, outboxEntries, receipts, now) {
...
function profileRisk(wallet, signedCount, submittedCount, failedReceiptCount, warnings) {
    let score = 100;
    const reasons = [];
...
function lowValuePolicy(wallet) {
    return wallet.lowValue;
}
function agentMetadata(wallet) {
    return wallet.agent;
}
function parseDecimal(input, decimals = 18) {
