...
export async function loadNodeRegistry(path) {
...
export function nodeRegistrySummary(loaded) {
...
export function listNodes(registry, args = {}) {
...
export function getNode(registry, id) {
...
export function nodeAttestation(node) {
...
export function explainPcr(node, pcr) {
    const pcrs = node.attestation?.pcrs ?? {};
    const expected = node.attestation?.expectedPcrs ?? {};
...
export function nodeDiversityScore(registry, args = {}) {
    const nodes = listNodes(registry, { ...args, limit: 500 });
    const asns = unique(nodes.map((node) => node.hosting?.asn).filter((value) => typeof value === "number").map(String));
    const providers = unique(nodes.map((node) => node.hosting?.provider).filter(Boolean));
    const countries = unique(nodes.map((node) => node.hosting?.country).filter(Boolean));
    const hostingClasses = unique(nodes.map((node) => node.hosting?.class).filter(Boolean));
    const operators = unique(nodes.map((node) => node.operatorId).filter(Boolean));
    const clusters = unique(nodes.map((node) => node.clusterId));
    const score = Math.min(100, Math.round(Math.min(asns.length, 8) * 12
...
export function nodeHostingClass(node) {
...
function hostingExplanation(klass) {
    switch (klass) {
