...
export function merchantPolicyStorePath() {
    return process.env.LYTH_MCP_MERCHANT_POLICY_STORE || join(homedir(), ".lyth_mcp", "merchant_policies.json");
}
export async function readMerchantPolicyStore(path = merchantPolicyStorePath()) {
    try {
...
export async function writeMerchantPolicyStore(store, path = merchantPolicyStorePath()) {
...
export async function upsertMerchantPolicy(patch) {
...
export async function getMerchantPolicy(vendorId) {
    return (await readMerchantPolicyStore()).policies.find((policy) => policy.vendorId === vendorId) ?? null;
}
export async function listMerchantPolicies(args = {}) {
...
export async function removeMerchantPolicy(vendorId) {
    const store = await readMerchantPolicyStore();
...
function normalizeList(values, uppercase) {
    if (!values) {
        return undefined;
    }
