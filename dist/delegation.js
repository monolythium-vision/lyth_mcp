...
import { clusterFoundationFlag, clusterReputation, getCluster, listClusters, } from "./clusters.js";
const PHASES = {
    bootstrap: {
        phase: "bootstrap",
...
export function delegationPhaseConfig(phase = "growth") {
    return PHASES[phase];
}
export function explainDelegationCaps(input = {}) {
...
export function stakeStatus(registry, input = {}) {
...
export function delegateDraft(registry, input) {
...
export function undelegateDraft(registry, input) {
    const cluster = getCluster(registry, input.clusterId);
...
export function rebalanceDraft(registry, input) {
    const positions = normalizePositions(input.positions ?? []);
    const total = positions.reduce((sum, position) => sum + parseAmount(position.amount), 0);
    const phase = delegationPhaseConfig(input.phase ?? "growth");
...
export function autovoteSimulate(registry, input) {
...
function safeCluster(registry, clusterId) {
    try {
...
function rationale(cluster, mode) {
    if (mode === "max_yield") {
